...
void simulate_game_mode(
    ...
    switch (game_mode) {
        case (GM_GAME): {
            ...
            // а когда получим от сервера его версию игры, то переключимся на
            // нее таким образом игра становиться более плавной и менее
            // зависящей от сервера если долго не будут приходить пакеты от
            // сервера, то клиент еще сможет играть
        ...
        case (GM_CUSTOMIZATION): {
            ...
            global_variables::camera.pos =
                customization_player.pos;  // static camera
    ...
    for (auto &player : game_variables::Players) {
        ...
        for (auto [sender_client_id, message, time] : ChatMessages) {
            ...
            if (player.client_id == sender_client_id) {
                ...
                draw_text_align(
                    message.c_str(),
                    player.pos - global_variables::camera.pos +
                        Dot(0, 30 + i * 10),
                    1,
                    Color(
                        0x00ff00,
                        static_cast<uint8_t>(message_alpha_factor * 0xff)
                    )
                );
                i++;
            }
...
void draw_game_mode(
    ...
    switch (game_mode) {
        case (GM_GAME): {
            ...
            if (now_is_typing) {
                draw_rect2(
                    -global_variables::arena_half_size + Dot(10, 20),
                    -global_variables::arena_half_size + Dot(162, 10), BLACK
                );
                efloat text_size = 1;
                /*efloat text_size = 150.0 /
                text_len_in_pixels(typer.get_text().c_str()); if (text_size > 1)
                { text_size = 1;
                }*/
                draw_text(
                    typer.text.c_str(),
        ...
        case (GM_CUSTOMIZATION): {
            ...
            // draw colors palette
            {
                ...
                for (int color_id = 0; color_id < count_colors; color_id++) {
                    if (customization_player.t_shirt_color_id == color_id) {
                        draw_rect(
                            start_pos + Dot(color_id * 8, 0), Dot(4, 4), WHITE
                        );
                    }
                    if (customization_player.cloack_color_id == color_id) {
                        draw_rect(
                            start_pos + Dot(color_id * 8, 0), Dot(3, 3), BLACK
                        );
                    }
                    draw_rect(
                        start_pos + Dot(color_id * 8, 0), Dot(2, 2),
            ...
            draw_text_align(
                "press T to change the choice of customization (t-shirt or "
                "cloack)",
                Dot(0, -30), 0.5, GREEN
            );
            draw_text_align(
                "press Q or E to change the choice of color", Dot(0, -40), 0.5,
                GREEN
            );
            draw_text_align(
                "use W,A,S,D to walk and SPACE to jump", Dot(0, -50), 0.5, GREEN
            );
            break;
        }
        case (GM_PAUSE_MENU): {
        ...
        case (GM_MULTIPLAYER_MENU): {
            ...
            draw_rect2(
                -global_variables::arena_half_size + Dot(50, 80),
