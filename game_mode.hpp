...
void simulate_game_mode(efloat delta_time, Player &player, Player &customization_player, WindowHandler &window_handler, Typer &typer) {
...
void draw_game_mode(efloat delta_time, int client_id, Player &customization_player, WindowHandler &window_handler, Typer &typer) {
    switch (game_mode) {
        case (GM_GAME): {
            ...
            if (now_is_typing) {
                draw_rect2(-global_variables::arena_half_size + Dot(10, 20), -global_variables::arena_half_size + Dot(162, 10), BLACK);
                efloat text_size = 1;
        ...
        case (GM_MULTIPLAYER_MENU): {
            ...
            draw_rect2(-global_variables::arena_half_size + Dot(50, 80), -global_variables::arena_half_size + Dot(200, 70), BLACK);
            // if (now_is_typing) {
