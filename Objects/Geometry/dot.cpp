...
Dot Dot::operator-() const { return {-x, -y}; }

Dot &Dot::operator+=(const Dot &p) { return *this = *this + p; }

Dot &Dot::operator-=(const Dot &p) { return *this = *this - p; }

Dot Dot::operator*(efloat k) const { return {x * k, y * k}; }

Dot &Dot::operator*=(efloat k) { return *this = *this * k; }

Dot Dot::operator/(efloat k) const { return {x / k, y / k}; }

Dot &Dot::operator/=(efloat k) { return *this = *this / k; }

efloat Dot::get_quare_len() const { return x * x + y * y; }
...
Dot operator*(efloat k, Dot p) { return p * k; }
...
bool operator!=(const Dot &lhs, const Dot &rhs) { return !(lhs == rhs); }
...
efloat get_angle(const Dot &a, const Dot &b) { return atan2(a % b, a * b); }
