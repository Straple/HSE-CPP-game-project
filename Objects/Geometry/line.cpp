...
efloat Line::get_a() const {
    return a;
}

[[nodiscard]] efloat Line::get_b() const {
    return b;
}

[[nodiscard]] efloat Line::get_c() const {
    return c;
}
...
Line::Line(efloat A, efloat B, efloat C)
    : a(A), b(B), c(C) {
    // normalize
    {
...
std::ostream &operator<<(std::ostream &output, const Line &l) {
    return output << "Line(" << l.get_a() << ", " << l.get_b() << ", " << l.get_c() << ')';
}
