...
const efloat get_performance_frequency() {
    static const efloat performance_frequency = static_cast<efloat>(calc_performance_frequency());
...
[[nodiscard]] efloat Timer::get() const {
    return static_cast<efloat>(get_ticks() - start_tick) / get_performance_frequency();
}
