 ...
 * p = позиция
 *
 * dp = p' = инерция движения, которую мы накапливаем, чтобы например когда
 * игрок идет вправо, а потом резко поворачивает налево, то он еще некоторое
 * время по инерции уходил вправо. А также, чтобы когда он только начал идти, то
 * сначала медленно, и с каждым разом чуть быстрее до некоторого значения.
 *
 * ddp = p'' = момент движения в кадре. Когда начинается кадр, то ddp = 0. Когда
 * игрок нажимает кнопку идти вправо, то ddp += C, а если влево то ddp -= C.
 * Таким образом, если он нажал и вправо и влево, то он никуда в итоге не
 * пойдет.
 *
 * После симуляции такого движения частичка ddp перейдет в dp, чтобы накопить
 * инерцию движения
 *
 ...
 * Это нужно, чтобы когда мы были слишком близко к точке p_to, то не постоянно
 * метались между ней, а сразу попали в неё
 */
...
// если вернет true, то эта точка грида доступна для посещения (например не
// мешают коллизии)
//
// suitable(Dot) -> bool
...
bool get_direction_to_shortest_path_bfs(const Dot &from,
                                        [[maybe_unused]] const Dot &to,
                                        Dot &result_dir, visitable_t visitable,
                                        suitable_t suitable
  ...
  if (find_answer) {
    ...
    while (find(starting_points.begin(), starting_points.end(),
                previous[answer_pos]) == starting_points.end()) {
...
template <typename visitable_t, typename suitable_t>
bool get_direction_to_shortest_path_Astar(const Dot &from, const Dot &to,
                                          std::vector<Dot> &path,
  ...
  std::priority_queue<std::pair<efloat, grid_pos_t>,
                      std::vector<std::pair<efloat, grid_pos_t>>,
                      std::greater<>>
      queue;
  std::map<grid_pos_t, efloat> dist;
  std::map<grid_pos_t, grid_pos_t> previous;
  ...
  while (!queue.empty()) {
    ...
    for (const auto &step : steps) {
      ...
      if (!dist.count(grid_neighbour) && visitable(grid_neighbour)) {
        ...
        queue.push(
            {dist[grid_neighbour] +
                 (cast_grid_coord_to_game_coord(grid_neighbour) - to).get_len(),
             grid_neighbour});
      }
    }
  }
  if (find_answer) {
    ...
    for (int i = q.size()-1; i >= 0; i--) {
        path[i] = (cast_grid_coord_to_game_coord(q.front()) - from);
        q.pop();
    }*/
  }
  // for (auto [grid_pos, d] : dist) {
...
template <typename visitable_t, typename suitable_t>
bool get_direction_to_shortest_path(const Dot &from, const Dot &to,
                                    std::vector<Dot> &path,
                                    visitable_t visitable, suitable_t suitable
                                    //,std::vector<Dot> &shortest_path,
                                    // std::vector<Dot> &grid
) {
  return get_direction_to_shortest_path_Astar(from, to, path, visitable,
                                              suitable);
}
