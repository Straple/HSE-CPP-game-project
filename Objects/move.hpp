...
bool get_direction_to_shortest_path_bfs(
    ...
    if (find_answer) {
        ...
        while (find(
                   starting_points.begin(), starting_points.end(),
                   previous[answer_pos]
               ) == starting_points.end()) {
        ...
        result_dir =
            (cast_grid_coord_to_game_coord(answer_pos) - from).normalize();
    }
...
bool get_direction_to_shortest_path_Astar(
    ...
    std::priority_queue<
        std::pair<efloat, grid_pos_t>,
        std::vector<std::pair<efloat, grid_pos_t>>, std::greater<>>
        queue;
    std::map<grid_pos_t, efloat> dist;
    std::map<grid_pos_t, grid_pos_t> previous;
    ...
    while (!queue.empty()) {
        ...
        for (const auto &step : steps) {
            ...
            if (!dist.count(grid_neighbour) && visitable(grid_neighbour)) {
                ...
                queue.push(
                    {dist[grid_neighbour] +
                         (cast_grid_coord_to_game_coord(grid_neighbour) - to)
                             .get_len(),
                     grid_neighbour}
                );
            }
...
bool get_direction_to_shortest_path(
    ...
    return get_direction_to_shortest_path_Astar(
        from, to, path, visitable, suitable
    );
}
