...
bool CollisionCircle::trigger(const CollisionCircle &other) const {
    return Circle(circle.pos, circle.radius + other.circle.radius)
        .isin(other.circle.pos);
}
...
CollisionCircle CollisionCircle::bubble(const CollisionCircle &other) const {
    return {
        CollisionCircle(circle.pos, circle.radius + other.circle.radius)
            .bubble(other.circle.pos),
        other.circle.radius};
}
...
std::unique_ptr<Collision> CollisionCircle::bubble(const Collision &other
) const {
    if (dynamic_cast<const CollisionCircle *>(&other) != nullptr) {
        return std::make_unique<CollisionCircle>(
            bubble(*dynamic_cast<const CollisionCircle *>(&other))
        );
    } else if (dynamic_cast<const CollisionBox *>(&other) != nullptr) {
        return std::make_unique<CollisionBox>(
            bubble(*dynamic_cast<const CollisionBox *>(&other))
        );
    } else {
        ASSERT(false, "who is other?");
    }
...
bool CollisionBox::trigger(const CollisionCircle &other) const {
    ...
    return CollisionBox(p0 + Dot(-radius, 0), p1 + Dot(radius, 0))
               .trigger(pos) ||
           CollisionBox(p0 + Dot(0, radius), p1 + Dot(0, -radius))
               .trigger(pos) ||
...
bool CollisionBox::trigger(const CollisionBox &other) const {
    return !(
        p1.x <= other.p0.x || p1.y >= other.p0.y || p0.x >= other.p1.x ||
        p0.y <= other.p1.y
    );
}
...
CollisionCircle CollisionBox::bubble(const CollisionCircle &other) const {
    ...
    // TODO: это не совсем корректно, в некоторых особо почти невозможных
    // случаях
...
CollisionBox CollisionBox::bubble(CollisionBox other) const {
    ...
    efloat min_shift =
        std::min({shift1_right, shift2_bottom, shift3_left, shift4_top});
