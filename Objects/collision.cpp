...
Dot CollisionCircle::get_pos() const { return circle.pos; }

bool CollisionCircle::trigger(const Dot &pos) const { return circle.isin(pos); }
...
CollisionCircle CollisionCircle::bubble(const CollisionCircle &other) const {
  return {CollisionCircle(circle.pos, circle.radius + other.circle.radius)
...
CollisionCircle::bubble(const Collision &other) const {
  if (dynamic_cast<const CollisionCircle *>(&other) != nullptr) {
...
Dot CollisionBox::get_pos() const { return p0; }
...
bool CollisionBox::trigger(const CollisionBox &other) const {
  return !(p1.x <= other.p0.x || p1.y >= other.p0.y || p0.x >= other.p1.x ||
           p0.y <= other.p1.y);
}
