...
#define ADD_BYTE_SERIALIZATION()                                         \
    ...
    inline void deserialize(std::istream &input_stream) {                \
        *this = byte_deserialization<                                    \
            std::remove_reference<decltype(*this)>::type>(input_stream); \
    }
...
struct serialization_traits<
    ...
    static void
    serialize(std::ostream &output_stream, const container_t &container) {
        byte_serialization<serialization_traits_container_size_t>(
            output_stream, container.size()
        );
    ...
    static container_t deserialize(std::istream &input_stream) {
        ...
        for (auto &value : temp_data) {
            value = serialization_traits<
                typename container_t::value_type>::deserialize(input_stream);
        }
