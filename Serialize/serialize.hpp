...
template <typename T> T byte_deserialization(std::istream &input_stream) {
  T result;
...
#define ADD_BYTE_SERIALIZATION()                                               \
  ...
  inline void deserialize(std::istream &input_stream) {                        \
    *this =                                                                    \
        byte_deserialization<std::remove_reference<decltype(*this)>::type>(    \
            input_stream);                                                     \
  }
...
struct serialization_traits<
    container_t,
    typename std::enable_if<is_container<container_t>::value>::type> {
  static void serialize(std::ostream &output_stream,
                        const container_t &container) {
    byte_serialization<serialization_traits_container_size_t>(output_stream,
                                                              container.size());
    for (const auto &value : container) {
      serialization_traits<typename container_t::value_type>::serialize(
          output_stream, value);
    }
  }

  static container_t deserialize(std::istream &input_stream) {
    serialization_traits_container_size_t size =
        byte_deserialization<decltype(size)>(input_stream);
    std::vector<typename container_t::value_type> temp_data(size);
    for (auto &value : temp_data) {
      value =
          serialization_traits<typename container_t::value_type>::deserialize(
              input_stream);
    }
