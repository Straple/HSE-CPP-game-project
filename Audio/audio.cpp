...
void init_musics(audiere::AudioDevicePtr &device, std::vector<std::string> Name) {
    ...
    for (auto it_name = Name.begin(); it_name != Name.end(); it_name++) {
        audio_variables::Musics[index].sound_ptr = OpenSound(device, ("Audio\\Musics\\" + *it_name).c_str(), true);
        index++;
    }
}
...
void init_sounds(audiere::AudioDevicePtr &device, std::vector<std::string> Name) {
    size_t index = 0;
    for (auto it_name = Name.begin(); it_name != Name.end(); it_name++) {
        audio_variables::Sounds[index].OpenSound(device, ("Audio\\Sounds\\" + *it_name).c_str(), true, 15);
        index++;
    }
}
...
void init_UI(audiere::AudioDevicePtr &device, std::vector<std::string> Name) {
    size_t index = 0;
    for (auto it_name = Name.begin(); it_name != Name.end(); it_name++) {
        audio_variables::UI[index].OpenSound(device, ("Audio\\Sounds\\" + *it_name).c_str(), true, 15);
        index++;
    }
}
...
void init_audio(audiere::AudioDevicePtr &device) {
    ...
    init_sounds(
        device, {"sniper_sound.mp3", "blaster.mp3", "magic.mp3", "rifle_shot.mp3", "slime_eating.wav", "explosion.mp3",
