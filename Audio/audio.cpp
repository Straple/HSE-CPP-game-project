...
#ifdef AUDIERE
void sound::OpenSound(audiere::AudioDevicePtr &device, const char *path,
                      bool streaming, size_t _size) {
  size = _size;
...
#ifdef AUDIERE
void UI_sound::OpenSound(audiere::AudioDevicePtr &device, const char *path,
                         bool streaming, size_t _size) {
  size = _size;
  sound_ptr = new audiere::OutputStreamPtr[_size];
  for (size_t i = 0; i < _size; i++) {
...
#ifdef AUDIERE
void init_musics(audiere::AudioDevicePtr &device,
...
void init_sounds(audiere::AudioDevicePtr &device,
                 std::vector<std::string> Name) {
  size_t index = 0;
  for (auto it_name = Name.begin(); it_name != Name.end(); it_name++) {
...
void init_audio(audiere::AudioDevicePtr &device) {
  ...
  init_sounds(device, {"sniper_sound.mp3", "blaster.mp3", "magic.mp3",
                       "rifle_shot.mp3", "slime_eating.wav", "explosion.mp3",
                       "run.mp3", "slime_shot.mp3"});
...
#ifdef AUDIERE
  for (int i = 0; i < MT_COUNT; i++) {
    audio_variables::Musics[i].sound_ptr->setVolume(music_volume *
                                                    global_volume);
  }
#endif
}
