...
struct animation {
  ...
  animation(spritesheet_t sprite_sheet, uint8_t frame_begin,
            uint8_t frame_count, efloat frame_duration);
  ...
  void draw(Dot pos, efloat size, func_t &&func = dummy_color_func()) const {
    draw_spritesheet(pos, size, sprite_sheet, frame_begin + frame_cur_count,
                     func);
  }
};
