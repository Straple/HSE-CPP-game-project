...
void draw_pixels_row_trivial_impl(Color *row, unsigned int len,
                                  const Color &color) {
  unsigned int x = 0;
  while (x < len) {
    unsigned int k = x + 1;
    ...
    fill(reinterpret_cast<unsigned int *>(row + x),
         static_cast<unsigned int>(row[x].combine(color)), k - x);
    x = k;
  }
}

void draw_pixels_impl(unsigned int x0, unsigned int y0, unsigned int x1,
                      unsigned int y1, const Color &color) {
  ...
  if (color.a == 0xff) {
    for (unsigned int y = y0; y < y1; y++, row += screen_width) {
      fill(reinterpret_cast<unsigned int *>(row),
...
// x0, x1 <= width
// y0, y1 <= height
void draw_pixels(unsigned int x0, unsigned int y0, unsigned int x1,
                 unsigned int y1, const Color &color) {
  if (x0 >= x1 || y0 >= y1) {
    return;
  }

  ASSERT(x0 <= global_variables::render_state.width() &&
...
void draw_rect_in_pixels(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                         const Color &color) {
  x0 = clamp<int64_t>(
...
void clear_screen(const Color &color) {
  if (color.a == 0xff) {
    ...
    fill(reinterpret_cast<unsigned int *>(pixels),
         static_cast<unsigned int>(color), size);
  } else {
    draw_pixels(0, 0, global_variables::render_state.width(),
