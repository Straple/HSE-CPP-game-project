...
void draw_circle(const Circle &circle, const Color &color) {
    ...
    for (int y = top_y; y <= bottom_y; y++) {
        efloat h =
            std::min(circle.radius, std::abs(y / global_variables::scale_factor - global_variables::arena_half_size.y - circle.pos.y));
...
void draw_rhombus(Dot pos, Dot half_size, const Color &color) {
    int top_y =
        clamp<int>(0, static_cast<int>((pos.y + global_variables::arena_half_size.y - half_size.y) * global_variables::scale_factor), static_cast<int>(global_variables::render_state.height()));
    int bottom_y =
        clamp<int>(0, static_cast<int>((pos.y + global_variables::arena_half_size.y + half_size.y) * global_variables::scale_factor), static_cast<int>(global_variables::render_state.height()) - 1);
