...
void draw_circle(const Circle &circle, const Color &color) {
  ...
  for (int y = top_y; y <= bottom_y; y++) {
    efloat h =
        std::min(circle.radius,
                 std::abs(y / global_variables::scale_factor -
...
void draw_rhombus(Dot pos, Dot half_size, const Color &color) {
  int top_y =
      clamp<int>(0,
                 static_cast<int>((pos.y + global_variables::arena_half_size.y -
                                   half_size.y) *
                                  global_variables::scale_factor),
                 static_cast<int>(global_variables::render_state.height()));
  int bottom_y =
  ...
  for (int y = top_y; y <= bottom_y; y++) {
    efloat cur_h = std::abs(y / global_variables::scale_factor -
                            global_variables::arena_half_size.y - pos.y);
    efloat delta = (half_size.y - cur_h) * half_size.x / half_size.y;

    int left_x = clamp<int>(
        0,
        static_cast<int>((pos.x + global_variables::arena_half_size.x - delta) *
                         global_variables::scale_factor),
        static_cast<int>(global_variables::render_state.width()));
    int right_x = clamp<int>(
        -1,
