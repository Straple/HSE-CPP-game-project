...
void draw_sprite_matrix(Dot pos, const efloat size, const Sprite &pixels,
  ...
  for (int i = 0; i < pixels.height(); i++) {
    for (int j = 0; j < pixels.width();) {
      ...
      if (is_draw(pixels[i][j])) {
        draw_rect_in_pixels(posx[j], posy[i + 1], posx[k], posy[i],
                            func(pixels[i][j]));
...
// рисует спрайт
template <typename func_t = dummy_color_func>
void draw_sprite_static(Dot pos, efloat size, sprite_t sprite,
                        func_t &&func = dummy_color_func()) {
  auto &pixels = get_sprite(sprite);

  if (arena_query(pos.x - size / 2, pos.x + pixels.width() * size + size / 2,
                  pos.y + size / 2,
...
template <typename func_t = dummy_color_func>
void draw_sprite(Dot pos, efloat size, sprite_t sprite,
                 func_t &&func = dummy_color_func()) {
...
bool collision_in_draw_sprite(Dot pos, efloat size, sprite_t sprite,
                              Dot collision_pos);
...
template <typename func_t = dummy_color_func>
void draw_texture(Dot pos, unsigned int x_cnt, unsigned int y_cnt, efloat size,
                  sprite_t texture, func_t &&func = dummy_color_func()) {
  ...
  for (unsigned int i = 0; i < y_cnt; i++) {
    for (unsigned int j = 0; j < x_cnt; j++) {
      draw_sprite(Dot(pos.x + j * x_summary, pos.y - i * y_summary), size,
                  texture, func);
    }
  }
}
...
// x_cnt, y_cnt - колво спрайтов по координатам
template <typename func_t = dummy_color_func>
void draw_texture_align(Dot pos, unsigned int x_cnt, unsigned int y_cnt,
                        efloat size, sprite_t texture,
                        func_t &&func = dummy_color_func()) {
  auto &pixels = get_sprite(texture);
  ...
  draw_texture(pos + 0.5 * Dot(-x_summary * x_cnt, y_summary * y_cnt), x_cnt,
...
template <typename func_t = dummy_color_func>
void draw_spritesheet_static(Dot pos, efloat size, spritesheet_t spritesheet,
  ...
  if (arena_query(
    ...
    draw_sprite_matrix(pos, size, get_spritesheet(spritesheet)[sprite_count],
                       func);
  }
}
...
// sprite_count - идентификатор спрайта
template <typename func_t = dummy_color_func>
void draw_spritesheet(Dot pos, efloat size, spritesheet_t spritesheet,
                      unsigned int sprite_count,
                      func_t &&func = dummy_color_func()) {
  static_pos_update(pos);
