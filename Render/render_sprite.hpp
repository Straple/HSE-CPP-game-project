...
void draw_sprite_static(Dot pos, efloat size, sprite_t sprite, func_t &&func = dummy_color_func()) {
    ...
    if (arena_query(pos.x - size / 2, pos.x + pixels.width() * size + size / 2, pos.y + size / 2, pos.y - pixels.height() * size - size / 2)) {
...
template <typename func_t = dummy_color_func>
void draw_texture(Dot pos, unsigned int x_cnt, unsigned int y_cnt, efloat size, sprite_t texture, func_t &&func = dummy_color_func()) {
...
// x_cnt, y_cnt - колво спрайтов по координатам
template <typename func_t = dummy_color_func>
void draw_texture_align(Dot pos, unsigned int x_cnt, unsigned int y_cnt, efloat size, sprite_t texture, func_t &&func = dummy_color_func()) {
    auto &pixels = get_sprite(texture);
...
template <typename func_t = dummy_color_func>
void draw_spritesheet_static(Dot pos, efloat size, spritesheet_t spritesheet, unsigned int sprite_count, func_t &&func = dummy_color_func()) {
...
// sprite_count - идентификатор спрайта
template <typename func_t = dummy_color_func>
void draw_spritesheet(Dot pos, efloat size, spritesheet_t spritesheet, unsigned int sprite_count, func_t &&func = dummy_color_func()) {
    static_pos_update(pos);
