...
void draw_collision(const CollisionCircle &collision,
                    const Color color = Color(0x00ff00, 70));

void draw_collision(const CollisionBox &collision,
                    const Color color = Color(0x00ff00, 70));

void draw_collision(const Collision &collision,
                    const Color color = Color(0x00ff00, 70));

template <typename T> void draw_collision(const T &object) {
  draw_collision(*object.get_collision());
}

void draw_hitbox(const Collision &collision);

template <typename T> void draw_hitbox(const T &object) {
  draw_hitbox(*object.get_hitbox());
}

template <typename T> void draw_hp(const T &object) {
