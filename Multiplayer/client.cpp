...
public:
  Client(boost::asio::io_context &io_context,
      ...
      : io_context(io_context), socket(io_context),
        ...
        time_tick_prev_frame(get_ticks()), window_handler(window_handler) {
  ...
  void send_input_to_server() {
    ...
    GameMessage message;
    message.set_body_length(sizeof(ButtonsState) + sizeof(Dot) +
                            2 * sizeof(int));
    ...
    if (game_mode == GM_GAME) {
      std::memcpy(message.body(), &window_handler.input.current,
                  sizeof(ButtonsState));
    ...
    std::memcpy(message.body() + sizeof(ButtonsState),
                &game_variables::Players[index].cursor_dir, sizeof(Dot));

    // запишем cloack_color_id
    std::memcpy(message.body() + sizeof(ButtonsState) + sizeof(Dot),
    ...
    boost::asio::async_connect(
        ...
        [this](boost::system::error_code error_code, tcp::endpoint) {
          if (!error_code) {
            ...
            do_read_header(); // мы должны получить наш client_id и игровой
                              // снапшот от сервера
          } else {
  ...
  void process_message_from_server() {
    if (read_message.message_type() == GameMessage::CLIENT_ID) {
      ...
      if (frame_id < game_state_frame_id) {
        ...
        std::memcpy(game_state.data(), read_message.body() + sizeof(int),
                    game_state.size());
        ...
        if (need_state_simulates_game_frame) {
          ...
          // мы еще ни разу не играли, так как сервер не отправил нам первое
          // игровое состояние
          simulate_game_frame();
        }
      } else {
      ...
      std::memcpy(chat_message.data(), read_message.body() + sizeof(int),
                  chat_message.size());
      ChatMessages.insert(ChatMessages.begin(),
                          {sender_client_id, chat_message, 0});
