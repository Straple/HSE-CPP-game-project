...
public:
    Client(boost::asio::io_context &io_context, const tcp::resolver::results_type &endpoints, WindowHandler &window_handler)
        : io_context(io_context), socket(io_context), timer(io_context, boost::posix_time::milliseconds(1)), time_tick_prev_frame(get_ticks()), window_handler(window_handler) {
    ...
    void send_input_to_server() {
        ...
        std::memcpy(message.body() + sizeof(ButtonsState) + sizeof(Dot), &customization_player.cloack_color_id, sizeof(int));
        // запишем t_shirt_color_id
        std::memcpy(message.body() + sizeof(ButtonsState) + sizeof(Dot) + sizeof(int), &customization_player.t_shirt_color_id, sizeof(int));
