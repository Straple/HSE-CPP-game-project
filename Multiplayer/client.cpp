...
void set_game_state(const std::string &game_state) {
    ...
    game_variables::Players =
        serialization_traits<std::vector<Player>>::deserialize(iss);

    // mobs
    game_variables::Slimes =
        serialization_traits<std::vector<Slime>>::deserialize(iss);
    game_variables::Bats =
        serialization_traits<std::vector<Bat>>::deserialize(iss);
    game_variables::Bombers =
        serialization_traits<std::vector<Bomber>>::deserialize(iss);

    // world
    game_variables::Trees =
        serialization_traits<std::vector<Tree>>::deserialize(iss);
    game_variables::Bushes =
        serialization_traits<std::vector<Bush>>::deserialize(iss);
    game_variables::Logs =
        serialization_traits<std::vector<Log>>::deserialize(iss);
    game_variables::Pillars =
        serialization_traits<std::vector<Pillar>>::deserialize(iss);
    game_variables::NunStatues =
        serialization_traits<std::vector<NunStatue>>::deserialize(iss);
    game_variables::Knights =
        serialization_traits<std::vector<Knight>>::deserialize(iss);
    game_variables::Barrels =
        serialization_traits<std::vector<Barrel>>::deserialize(iss);

    // loot
    game_variables::Loot_coins =
        serialization_traits<std::vector<Coin>>::deserialize(iss);
    game_variables::Loot_hearts =
        serialization_traits<std::vector<Heart>>::deserialize(iss);

    game_variables::Effects =
        serialization_traits<std::vector<Effect>>::deserialize(iss);

    game_variables::Bullets =
        serialization_traits<std::vector<Bullet>>::deserialize(iss);
    game_variables::Weapons =
        serialization_traits<std::vector<Weapon>>::deserialize(iss);
}
...
public:
    ...
    void send_input_to_server() {
        ...
        if (game_mode == GM_GAME) {
            std::memcpy(
                message.body(), &window_handler.input.current,
                sizeof(ButtonsState)
            );
        ...
        std::memcpy(
            message.body() + sizeof(ButtonsState),
            &game_variables::Players[index].cursor_dir, sizeof(Dot)
        );

        // запишем cloack_color_id
        std::memcpy(
            message.body() + sizeof(ButtonsState) + sizeof(Dot),
            &customization_player.cloack_color_id, sizeof(int)
        );
        // запишем t_shirt_color_id
        std::memcpy(
            message.body() + sizeof(ButtonsState) + sizeof(Dot) + sizeof(int),
            &customization_player.t_shirt_color_id, sizeof(int)
        );
    ...
    void simulate_game_frame() {
        ...
        // update debug info
        {
            ...
            if (accum_time_measurement >= 1) {
                ...
                count_of_non_server_frames_snapshot =
                    std::exchange(count_of_non_server_frames_accum, 0);
            }
        }
        ...
        simulate_game_mode(
            delta_time, player, customization_player, window_handler, typer
        );

        // мы сбросили сообщение чата и оно не было пусто
        if (old_is_typing != now_is_typing && old_is_typing &&
            !typer_text.empty()) {
        ...
        draw_game_mode(
            delta_time, client_id, customization_player, window_handler, typer
        );
        ...
        timer.expires_at(
            timer.expires_at() + boost::posix_time::milliseconds(1)
        );
...
private:
    void do_connect(const tcp::resolver::results_type &endpoints) {
        boost::asio::async_connect(
            ...
            [this](boost::system::error_code error_code, tcp::endpoint) {
                if (!error_code) {
                    std::cout << "connecting successful. message: \""
                              << error_code.message() << "\"" << std::endl;
                    do_read_header();  // мы должны получить наш client_id и
                                       // игровой снапшот от сервера
                } else {
                    std::cout << "connecting failed. message: \""
                              << error_code.message() << "\"" << std::endl;
                    close();
                }
            }
    ...
    void process_message_from_server() {
        if (read_message.message_type() == GameMessage::CLIENT_ID) {
            ...
            if (frame_id < game_state_frame_id) {
                ...
                std::memcpy(
                    game_state.data(), read_message.body() + sizeof(int),
                    game_state.size()
                );
                ...
                if (need_state_simulates_game_frame) {
                    ...
                    // мы еще ни разу не играли, так как сервер не отправил нам
            ...
            std::memcpy(
                chat_message.data(), read_message.body() + sizeof(int),
                chat_message.size()
            );
            ChatMessages.insert(
                ChatMessages.begin(), {sender_client_id, chat_message, 0}
            );

        } else if (read_message.message_type() == GameMessage::SOUNDS) {
            int size = read_message.body_length();
            for (int i = 0; i < size; i++) {
                Audio::sound_type type = *reinterpret_cast<Audio::sound_type *>(
                    read_message.body() + i * sizeof(uint8_t)
                );
    ...
    // этот кадр от сервера? если да, то текущее состояние игры мы получили от
    // сервера
...
std::endl;
        ...
        ShowWindow(GetConsoleWindow(), global_variables::show_console ? SW_SHOW
: SW_HIDE); ShowCursor(global_variables::show_cursor);
