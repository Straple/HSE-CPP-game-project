...
std::string get_game_state() {
  ...
  serialization_traits<std::vector<Player>>::serialize(oss,
                                                       game_variables::Players);

  // mobs
  serialization_traits<std::vector<Slime>>::serialize(oss,
                                                      game_variables::Slimes);
  serialization_traits<std::vector<Bat>>::serialize(oss, game_variables::Bats);
  serialization_traits<std::vector<Bomber>>::serialize(oss,
                                                       game_variables::Bombers);

  // world
  serialization_traits<std::vector<Tree>>::serialize(oss,
                                                     game_variables::Trees);
  serialization_traits<std::vector<Bush>>::serialize(oss,
                                                     game_variables::Bushes);
  serialization_traits<std::vector<Log>>::serialize(oss, game_variables::Logs);
  serialization_traits<std::vector<Pillar>>::serialize(oss,
                                                       game_variables::Pillars);
  serialization_traits<std::vector<NunStatue>>::serialize(
      oss, game_variables::NunStatues);
  serialization_traits<std::vector<Knight>>::serialize(oss,
                                                       game_variables::Knights);
  serialization_traits<std::vector<Barrel>>::serialize(oss,
                                                       game_variables::Barrels);
  ...
  serialization_traits<std::vector<Effect>>::serialize(oss,
                                                       game_variables::Effects);

  // weapons
  serialization_traits<std::vector<Bullet>>::serialize(oss,
                                                       game_variables::Bullets);
  serialization_traits<std::vector<Weapon>>::serialize(oss,
                                                       game_variables::Weapons);
...
GameMessage build_sounds_message(const std::vector<Audio::sound_type> &Sounds) {
  ...
  // std::cout << "new sounds message | body length: " << message.body_length()
...
GameMessage build_game_state_message(int game_frame_id) {
  ...
  std::memcpy(message.body() + sizeof(int), game_state_str.data(),
              game_state_str.size());
  return message;
}
...
public:
  ...
  void simulate(efloat delta_time) { simulate_player(delta_time, client_id); }

  virtual void add_sounds(const std::vector<Audio::sound_type> &Sounds) = 0;

  virtual void deliver(const GameMessage &message) = 0;

  virtual void add_chat_message_in_queue(int client_id,
...
public:
  explicit GameSession(boost::asio::io_context &io_context)
      : timer_for_simulate_game(io_context, boost::posix_time::milliseconds(0)),
        timer_for_print_debug_info(io_context,
  ...
  void simulate_game_frame() {
    ...
    // update time
    {
      ...
      delta_time = static_cast<efloat>(cur_time_tick - time_tick_prev_frame) /
    ...
    timer_for_simulate_game.expires_at(timer_for_simulate_game.expires_at() +
  ...
  void join(const AbstractClientPtr &client_ptr) {
    ...
    game_variables::Players.emplace_back(
        player_teleport_dots[get_random_engine()() %
                             player_teleport_dots.size()]);
    ...
    // клиенту, а потом мы не обновив game_state_snapshot отправим его уже без
    // этого игрокав
    frame_id++;
    game_state_snapshot = build_game_state_message(frame_id);
  }
  ...
  void leave(const AbstractClientPtr &client_ptr) {
    ...
    // (из-за асинхронности куча, ожидающих выполнения действий, придут сюда из
    // ClientHandler, чтобы уйти из сессии)
    if (Clients.find(client_ptr) != Clients.end()) {
      ...
      // так как например, если слайм ел игрока, а он вышел из игры, то кого он
      // в итоге ест? но да ладно, мы скоро его все равно пересчитаем
    }
  }
...
public:
  ...
  void add_chat_message_in_queue(int client_id,
...
private:
  void do_read_header() {
    ...
    boost::asio::async_read(
        socket,
        boost::asio::buffer(read_message.data(), GameMessage::header_length),
  ...
  void do_read_body() {
    ...
    boost::asio::async_read(
        socket,
        boost::asio::buffer(read_message.body(), read_message.body_length()),
        [this, self](boost::system::error_code error_code, std::size_t) {
          ...
          if (!error_code) {
            ...
            if (read_message.message_type() == GameMessage::PLAYER_INPUT) {
              set_player_input();
            } else if (read_message.message_type() ==
                       GameMessage::CHAT_MESSAGE) {
              std::string chat_message;
              chat_message.resize(read_message.body_length());
              std::memcpy(chat_message.data(), read_message.body(),
                          chat_message.size());
              session.add_chat_message(client_id, chat_message);
              std::cout << "client_id: " << client_id << " chat message: \""
                        << chat_message << '\"' << std::endl;
            }
  ...
  void set_player_input() {
    ASSERT(read_message.message_type() == GameMessage::PLAYER_INPUT &&
        ...
        [this, self](boost::system::error_code error_code, std::size_t) {
          ...
          if (!error_code) {
            if (!SoundsQueue.empty()) {
              ...
              std::memcpy(message.body() + sizeof(int), chat_message.data(),
                          chat_message.size());
              deliver(message);
            }
          } else {
    ...
    acceptor.async_accept(
        [this](boost::system::error_code error_code, tcp::socket socket) {
          ...
          if (!error_code) {
            ...
            std::make_shared<ClientHandler>(std::move(socket), session,
                                            count_of_connects)
                ->start();
            count_of_connects++;
          }
