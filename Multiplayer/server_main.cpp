...
std::string get_game_state() {
    ...
    serialization_traits<std::vector<Player>>::serialize(
        oss, game_variables::Players
    );

    // mobs
    serialization_traits<std::vector<Slime>>::serialize(
        oss, game_variables::Slimes
    );
    serialization_traits<std::vector<Bat>>::serialize(
        oss, game_variables::Bats
    );
    serialization_traits<std::vector<Bomber>>::serialize(
        oss, game_variables::Bombers
    );

    // world
    serialization_traits<std::vector<Tree>>::serialize(
        oss, game_variables::Trees
    );
    serialization_traits<std::vector<Bush>>::serialize(
        oss, game_variables::Bushes
    );
    serialization_traits<std::vector<Log>>::serialize(
        oss, game_variables::Logs
    );
    serialization_traits<std::vector<Pillar>>::serialize(
        oss, game_variables::Pillars
    );
    serialization_traits<std::vector<NunStatue>>::serialize(
        oss, game_variables::NunStatues
    );
    serialization_traits<std::vector<Knight>>::serialize(
        oss, game_variables::Knights
    );
    serialization_traits<std::vector<Barrel>>::serialize(
        oss, game_variables::Barrels
    );

    // loot
    serialization_traits<std::vector<Coin>>::serialize(
        oss, game_variables::Loot_coins
    );
    serialization_traits<std::vector<Heart>>::serialize(
        oss, game_variables::Loot_hearts
    );

    serialization_traits<std::vector<Effect>>::serialize(
        oss, game_variables::Effects
    );

    // weapons
    serialization_traits<std::vector<Bullet>>::serialize(
        oss, game_variables::Bullets
    );
    serialization_traits<std::vector<Weapon>>::serialize(
        oss, game_variables::Weapons
    );
...
GameMessage build_sounds_message(const std::vector<Audio::sound_type> &Sounds) {
    ...
    // std::cout << "new sounds message | body length: " <<
    // message.body_length()
    // << " Sounds.size(): " << Sounds.size()
    //           << '\n';
    return message;
}
...
GameMessage build_game_state_message(int game_frame_id) {
    ...
    std::memcpy(
        message.body() + sizeof(int), game_state_str.data(),
        game_state_str.size()
    );
    return message;
}
...
public:
    ...
    virtual void add_chat_message_in_queue(
        int client_id,
...
public:
    explicit GameSession(boost::asio::io_context &io_context)
        : timer_for_simulate_game(
              io_context,
              boost::posix_time::milliseconds(0)
          ),
          timer_for_print_debug_info(
              io_context,
              boost::posix_time::milliseconds(0)
          ),
    ...
    void print_debug_info() {
        std::cout << "server fps: " << frame_count_accum << ' '
                  << "frame_id: " << frame_id << '\n';
        if (!Clients.empty()) {
            std::cout << "write | reade\n";
        }
        for (auto &client_ptr : Clients) {
            std::cout << client_ptr->count_of_write_messages << ' '
                      << client_ptr->count_of_read_messages << '\n';
        ...
        // продлеваем таймер
        timer_for_print_debug_info.expires_at(
            timer_for_print_debug_info.expires_at() +
            boost::posix_time::seconds(1)
        );
        timer_for_print_debug_info.async_wait(
            boost::bind(&GameSession::print_debug_info, this)
        );
    }
    ...
    void simulate_game_frame() {
        ...
        // update time
        {
            ...
            delta_time =
                static_cast<efloat>(cur_time_tick - time_tick_prev_frame) /
        ...
        if (test_room.wave_number == 2) {
            ...
            // отправляем клиентам сообщение о том, что нужно взять новый
            // уровень
            for (const auto &client_ptr : Clients) {
        ...
        timer_for_simulate_game.expires_at(
            timer_for_simulate_game.expires_at() +
            boost::posix_time::milliseconds(16)
        );
        timer_for_simulate_game.async_wait(
            boost::bind(&GameSession::simulate_game_frame, this)
        );
    }
    ...
    void join(const AbstractClientPtr &client_ptr) {
        ...
        game_variables::Players.emplace_back(
            player_teleport_dots
                [get_random_engine()() % player_teleport_dots.size()]
        );
        game_variables::Players.back().client_id = client_ptr->client_id;

        // обновим игровой снапшот, так как мы получили нового игрока
        // иначе будет UB: после join() ClientHandler отправит get_game_state()
        // клиенту, а потом мы не обновив game_state_snapshot отправим его уже
        // без этого игрокав
        frame_id++;
        game_state_snapshot = build_game_state_message(frame_id);
    }
    ...
    void leave(const AbstractClientPtr &client_ptr) {
        ...
        // (из-за асинхронности куча, ожидающих выполнения действий, придут сюда
        // из ClientHandler, чтобы уйти из сессии)
        if (Clients.find(client_ptr) != Clients.end()) {
            ...
            game_variables::Players.erase(
                game_variables::Players.begin() + index
            );

            // TODO: возможно тут стоит тоже сразу же пересчитать игровой мир
            // так как например, если слайм ел игрока, а он вышел из игры, то
            // кого он в итоге ест? но да ладно, мы скоро его все равно
            // пересчитаем
        }
    }
...
class ClientHandler : public AbstractClient,
...
public:
    ClientHandler(tcp::socket socket, GameSession &session, const int client_id)
        ...
        // потому что мы еще не построили себя => не построили
        // shared_from_this() поэтому будет error: bad_weak_ptr
    }
    ...
    void add_chat_message_in_queue(
        int client_id,
        const std::string &chat_message
    ) override {
...
private:
    void do_read_header() {
        ...
        boost::asio::async_read(
            socket,
            boost::asio::buffer(
                read_message.data(), GameMessage::header_length
            ),
            [this, self](boost::system::error_code error_code, std::size_t) {
                if (!error_code) {
                    ...
                    std::cout << "reading header failed. message: \""
        ...
        boost::asio::async_read(
            ...
            [this, self](boost::system::error_code error_code, std::size_t) {
                ...
                if (!error_code) {
                    ...
                    std::cout << "reading body failed. message: \""
                              << error_code.message() << "\"" << std::endl;
                    session.leave(shared_from_this());
                }
            }
    ...
    void set_player_input() {
        ASSERT(
            read_message.message_type() == GameMessage::PLAYER_INPUT &&
                read_message.body_length() ==
                    sizeof(ButtonsState) + sizeof(Dot) + 2 * sizeof(int),
            ...
            [this, self](boost::system::error_code error_code, std::size_t) {
                ...
                if (!error_code) {
                    if (!SoundsQueue.empty()) {
                        ...
                        GameMessage sounds_message =
                            build_sounds_message(SoundsQueue);
                        SoundsQueue.clear();
                        deliver(sounds_message);
                    } else if (!queue_chat_messages.empty()) {
                        // отправим сообщение из чата клиенту
                        auto [sender_client_id, chat_message] =
                            queue_chat_messages.front();
                        queue_chat_messages.pop();

                        GameMessage message;
                        message.set_body_length(
                            sizeof(int) + chat_message.size()
                        );
                        message.set_message_type(GameMessage::CHAT_MESSAGE);
                        message.encode_header();

                        std::memcpy(
                            message.body(), &sender_client_id, sizeof(int)
                        );
                        std::memcpy(
                            message.body() + sizeof(int), chat_message.data(),
                            chat_message.size()
                        );
                        deliver(message);
                    }
    ...
    std::vector<Audio::sound_type>
        SoundsQueue;  // очередь звуков, которые нужно отправить клиенту
};
...
private:
            ...
            [this](boost::system::error_code error_code, tcp::socket socket) {
                {
                    ...
                    std::cout << "Connected " << remote_endpoint << " --> "
                              << local_endpoint << " message: \""
                              << error_code.message() << "\"\n\n";
                    std::cout.flush();
                }
                // если нет ошибок сокета
                if (!error_code) {
                    // создать клиенту его обработчика и добавить в игровую
                    // сессию
                    std::make_shared<ClientHandler>(
                        std::move(socket), session, count_of_connects
                    )
                        ->start();
                    count_of_connects++;
                }
    ...
    int count_of_connects = 0;  // количество подключений, используется для
                                // формирования уникального id клиента
};
