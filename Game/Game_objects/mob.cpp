...
void Mob::update_move_dir(efloat delta_time, Dot player_pos,
...
void Mob::simulate_move_to_player(
    Dot player_pos, const std::set<grid_pos_t> &visitable_grid_dots,
    efloat delta_time) {
  ...
  if ((pos - to).get_len() < 30) {
    ...
    if (path_size != 0) {
      Dot save_dp = dp;
      simulate_move_to2d(pos, path_prefix[0], dp,
                         (path_prefix[0] - pos).normalize() * 400, delta_time);
    ...
    if (!have_visitable_dot_near) {
        ...
        //    int cur_dist = std::abs(cur_grid_pos.first -
    visitable_grid_pos.first) +
        //                   std::abs(cur_grid_pos.second -
    visitable_grid_pos.second);
        ...
        //pos = cast_grid_coord_to_game_coord(best_grid_pos);
                   ...
                   [&](const grid_pos_t &request) { return
    visitable_grid_dots.count(request); },
                   [&](const Dot &request) { return (to - request).get_len() <
    15; }
               )) {
