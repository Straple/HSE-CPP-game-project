...
struct Panel {
    ...
    Panel(
        ...
        : pos(pos_), size(size_), text(std::move(text_)), is_interactive(is_interactive_), color(color_), function(function_) {
...
struct Menu {
    ...
    Menu(std::vector<Panel> panels_)
        : panels(std::move(panels_)) {
    }
