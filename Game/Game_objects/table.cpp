...
Table::Table(const Dot new_pos) { pos = new_pos; }

[[nodiscard]] std::unique_ptr<Collision> Table::get_collision() const {
  return std::make_unique<CollisionBox>(pos + delta_draw_pos + Dot(0, -5),
                                        pos + Dot(-delta_draw_pos.x, 0));
}
...
bool Table::trigger_in_draw_sprite(const Dot &check_pos) {
  return collision_in_draw_sprite(pos + delta_draw_pos, size, SP_TABLE,
                                  check_pos);
}