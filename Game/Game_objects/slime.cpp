...
void Slime::simulate(const efloat delta_time,
  ...
  if (paralyzed_accum > 0) {
    ...
    if (anim.frame_update(delta_time)) {
      ...
      game_variables::Bullets.emplace_back(BulletHostType::ENEMY, bullet_pos,
                                           game_variables::Players[index].pos,
                                           1, 1000, SP_SLIME_BULLET);
    }
  } else {
    ...
    //        simulate_move_to2d(pos, pos + move_dir_to_target, dp,
    //        move_dir_to_target * ddp_speed, delta_time);
...
void Slime::draw() {
  if (is_devour) {
    if (is_between<uint8_t>(9, anim.frame_count, 25)) {
      draw_sprite(pos + delta_draw_pos, size, SP_SLIME_LARGE_SHADOW,
                  shadow_color_func());
    } else {
      draw_sprite(pos + delta_draw_pos, size, SP_SLIME_MEDIUM_SHADOW,
                  shadow_color_func());
    }
    ...
    anim.draw(pos + delta_draw_pos, size, [&](Color color) {
      return paralyzed_accum > 0
                 ? Color(0xffffff, 128)
                 : Color(static_cast<uint32_t>(color), draw_alpha);
    });
  }
  ...
  /*for (int i = 0; i < path_size; i++) {
      draw_rect(path_prefix[i]-global_variables::camera.pos, Dot(0.5,0.5),
  GREEN);
  }
