...
void Room::build_grid() {
  ...
  while (!queue.empty()) {
    ...
    for (const auto &step : steps) {
      grid_pos_t grid_neighbour = {grid_pos.first + step.first,
                                   grid_pos.second + step.second};
      Dot pos = grid_start_dot +
                step_size * Dot(grid_neighbour.first, grid_neighbour.second);
...
std::vector<std::pair<Dot, object_type>>
Room::read(const std::string &filename) {
  // clear
  {
    Draw_objects.clear();
    Walls.clear();
...

  {
    ...
    for (auto &player : game_variables::Players) {
      player.pos = player_teleport_dots[get_random_engine()() %
...
void Room::simulate(efloat delta_time) {
      ...
      for (auto [pos, name] : Interesting_dots) {
        if (name.size() < 5 || name.substr(0, 6) != "player") {
          if (name == "sniper_rifle") {
            if (randomness(30)) {
              game_variables::Weapons.emplace_back(pos, weapon_t::SNIPER_RIFLE,
                                                   3);
            }
          } else if (name == "staff") {
            if (randomness(50)) {
  ...
  for (int i = 0; i < static_cast<int>(game_variables::Bombers.size()); i++) {
    game_variables::Bombers[i].simulate(delta_time,
                                        visitable_grid_dots_for_ground_mob);
    ...
    if (bomber1.is_booming && bomber1.anim.frame_cur_count > 5 &&
      ...
      for (auto &bomber2 : game_variables::Bombers) {
        if (!bomber2.is_booming &&
            (bomber1.pos - bomber2.pos).get_len() < Bomber::boom_radius) {
          // цепная реакция!
          bomber2.do_boom();
        }
      }
  ...
  // simulate effects
  {
    for (int i = 0; i < game_variables::Effects.size(); i++) {
      if (game_variables::Effects[i].simulate(delta_time)) {
        ...
        i--;
      } else if (game_variables::Effects[i].anim.sprite_sheet ==
                 SS_FLOWER_EFFECT) {
  ...
  // simulate loot
  {
    // heart
    {
      ...
      for (int i = 0; i < static_cast<int>(game_variables::Loot_hearts.size());
           i++) {
    ...
    // coin
    {
      ...
      for (int i = 0; i < static_cast<int>(game_variables::Loot_coins.size());
           i++) {
        auto &coin = game_variables::Loot_coins[i];
        if (coin.simulate_collection()) {
          game_variables::Loot_coins.erase(game_variables::Loot_coins.begin() +
                                           i);
          i--;
        }
      }
...
void Room::draw() {
  ...
  // top sort
  {
    ...
    for (auto &effect : game_variables::Effects) {
      if (effect.anim.sprite_sheet == SS_FLOWER_EFFECT) {
        draw_circle(Circle(effect.pos - global_variables::camera.pos, 5),
                    Color(0x00ff00, 128));
  ...
  for (auto [top_left, bottom_right, color] : ColorBoxes) {
    draw_rect2(top_left - global_variables::camera.pos,
