...
void Room::build_grid() {
    ...
    const static std::vector<grid_pos_t> steps = {
    ...
    while (!queue.empty()) {
        ...
        for (const auto &step : steps) {
            grid_pos_t grid_neighbour = {
                grid_pos.first + step.first, grid_pos.second + step.second};
            Dot pos = grid_start_dot + step_size * Dot(grid_neighbour.first,
                                                       grid_neighbour.second);
            ...
            if (!visitable_grid_dots_for_air_mob.count(grid_neighbour)) {
                ...
                for (const auto &tree : game_variables::Trees) {
                    if (tree.get_collision()->trigger(
                            *slime_tester.get_collision()
                        )) {
                        is_visitable = false;
                        break;
                    }
                ...
                for (const auto &pillar : game_variables::Pillars) {
                    if (pillar.get_collision()->trigger(
                            *slime_tester.get_collision()
                        )) {
                        is_visitable = false;
                        break;
                ...
                for (const auto &statue : game_variables::NunStatues) {
                    if (statue.get_collision()->trigger(
                            *slime_tester.get_collision()
                        )) {
                        is_visitable = false;
                        break;
    ...
    // грид для наземных это грид для воздушных - те точки, в которых мы быть не
    // можем
...
std::vector<std::pair<Dot, object_type>> Room::read(const std::string &filename
...

    {
        ...
        for (auto &player : game_variables::Players) {
            player.pos = player_teleport_dots
    ...
    std::cout << "visitable grid dots for air mob: "
              << visitable_grid_dots_for_air_mob.size() << std::endl;
    std::cout << "visitable grid dots for ground mob: "
              << visitable_grid_dots_for_ground_mob.size() << std::endl;
...
void Room::write(
    ...
    const std::vector<std::pair<Dot, object_type>> &Objects
) {
...
void Room::simulate(efloat delta_time) {
    ...
    if (game_variables::Slimes.size() + game_variables::Bats.size() +
            ...
            for (auto [pos, name] : Interesting_dots) {
                if (name.size() < 5 || name.substr(0, 6) != "player") {
                    if (name == "sniper_rifle") {
                        if (randomness(30)) {
                            game_variables::Weapons.emplace_back(
                                pos, weapon_t::SNIPER_RIFLE, 3
                            );
                        }
                    } else if (name == "staff") {
                        if (randomness(50)) {
        ...
        weapon_t::SNIPER_RIFLE, 3); } else if (name == "staff") {
                    game_variables::Weapons.emplace_back(pos, STAFF);
                } else if (name == "golden_gun") {
    ...
    for (int i = 0; i < static_cast<int>(game_variables::Bombers.size()); i++) {
        game_variables::Bombers[i].simulate(
            delta_time, visitable_grid_dots_for_ground_mob
        );
    ...
    for (auto &bomber1 : game_variables::Bombers) {
        if (bomber1.is_booming && bomber1.anim.frame_cur_count > 5 &&
            bomber1.anim.frame_cur_count < 12) {
            for (auto &bomber2 : game_variables::Bombers) {
                if (!bomber2.is_booming &&
                    (bomber1.pos - bomber2.pos).get_len() <
                        Bomber::boom_radius) {
                    // цепная реакция!
                    bomber2.do_boom();
                }
    ...
    // simulate bullets
    {
        ...
        for (uint32_t i = 0; i < game_variables::Bullets.size(); i++) {
            if (game_variables::Bullets[i].simulate_attack_on_mob(
                    game_variables::Slimes
                ) ||
                game_variables::Bullets[i].simulate_attack_on_mob(
                    game_variables::Bats
                ) ||
                game_variables::Bullets[i].simulate_attack_on_mob(
                    game_variables::Bombers
                ) ||
                game_variables::Bullets[i].simulate_attack_on_player(
                    game_variables::Players
                )) {
                game_variables::Bullets.erase(
                    game_variables::Bullets.begin() + i
                );
                i--;
            }
        }
    ...
    // simulate effects
    {
        for (int i = 0; i < game_variables::Effects.size(); i++) {
            if (game_variables::Effects[i].simulate(delta_time)) {
                game_variables::Effects.erase(
                    game_variables::Effects.begin() + i
                );
                i--;
    ...
    // simulate loot
    {
        // heart
        {
            ...
            for (int i = 0;
                 i < static_cast<int>(game_variables::Loot_hearts.size());
                 i++) {
                auto &heart = game_variables::Loot_hearts[i];
                if (heart.simulate_collection()) {
                    game_variables::Loot_hearts.erase(
                        game_variables::Loot_hearts.begin() + i
                    );
                    i--;
                }
            }
        ...
        // coin
        {
            ...
            for (int i = 0;
                 i < static_cast<int>(game_variables::Loot_coins.size()); i++) {
                auto &coin = game_variables::Loot_coins[i];
                if (coin.simulate_collection()) {
                    game_variables::Loot_coins.erase(
                        game_variables::Loot_coins.begin() + i
                    );
                    i--;
                }
            }
...
void Room::draw() {
    ...
    // top sort
    {
        ...
        for (auto &effect : game_variables::Effects) {
            if (effect.anim.sprite_sheet == SS_FLOWER_EFFECT) {
                draw_circle(
                    Circle(effect.pos - global_variables::camera.pos, 5),
                    Color(0x00ff00, 128)
                );
        ...
        std::stable_sort(
            ...
            [](AbstractObject *lhs, AbstractObject *rhs) {
    ...
    {
        // draw grid start
        /*draw_rect(grid_start_dot - global_variables::camera.pos, Dot(1, 1),
        YELLOW);

        for(auto grid_pos : visitable_grid_dots){
            Dot pos = grid_start_dot + step_size * Dot(grid_pos.first,
        grid_pos.second); draw_rect(pos - global_variables::camera.pos, Dot(0.5,
        0.5), YELLOW);
        }*/
    }
    ...
    for (auto [top_left, bottom_right, color] : ColorBoxes) {
        draw_rect2(
            top_left - global_variables::camera.pos,
            bottom_right - global_variables::camera.pos, color
        );
    }
}
