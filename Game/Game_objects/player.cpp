...
void Player::reborn() {
    // Аналогично, можно не только для возрождения но и для
    // появления/телепортации использовать
    set_invulnerable();
    hp = 1;
}
...
int find_nearest_player_index(Dot pos) {
    int best = -1;
    for (std::size_t index = 0; index < game_variables::Players.size();
         index++) {
        ...
        if (best == -1 ||
            (game_variables::Players[index].pos - pos).get_len() <
