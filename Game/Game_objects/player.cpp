...
// возвращает анимацию
Player_anim_tree::anim_and_dir_t
Player_anim_tree::get_anim(Dot dir, anim_and_dir_t type) {
  // дефолтная анимация
  if (type.anim == NONE) {
...
[[nodiscard]] bool Player::is_dead() const { return hp <= 0; }
...
void Player::reborn() {
  // Аналогично, можно не только для возрождения но и для появления/телепортации
  // использовать
  set_invulnerable();
  hp = 1;
}
...
void Player::drop_weapon() {
  if (weapon_ind != -1) {
    game_variables::Weapons[weapon_ind].pos -=
        game_variables::Weapons[weapon_ind].delta;
...
void Player::set_invulnerable() { invulnerable_accum = invulnerable_cooldown; }
...
int find_player_index(int client_id) {
  for (std::size_t index = 0;
       index < game_variables::game_variables::Players.size(); index++) {
...
int find_nearest_player_index(Dot pos) {
  ...
  for (std::size_t index = 0; index < game_variables::Players.size(); index++) {
    if (game_variables::Players[index].is_dead() ||
        game_variables::Players[index].is_paralyzed) {
      continue;
    }
