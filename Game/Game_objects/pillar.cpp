...
Pillar::Pillar(const Dot &position) { pos = position; }
...
void Pillar::draw() {
  draw_rect2(pos + Dot(-14, 8) - global_variables::camera.pos,
             pos + Dot(14, -12) - global_variables::camera.pos, Color(0, 128));
...
bool Pillar::trigger_in_draw_sprite(const Dot &check_pos) {
  return collision_in_draw_sprite(pos + delta_draw_pos, size, SP_PILLAR,
                                  check_pos);
}