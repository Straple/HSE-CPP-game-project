    ...
    for (int i = 0; i < Enemies.size(); i++) {
        if (!Enemies[i].is_invulnerable() &&
            ...
            if (Enemies[i].hp <= 0) {
                ...
                if (randomness(20)) {
                    game_variables::Loot_hearts.push_back(Heart(
                        Enemies[i].get_hitbox()->get_pos(), ddp.normalize()
                    ));
                } else if (randomness(50)) {
                    for (int count = 0; count < 4; count++) {
                        game_variables::Loot_coins.push_back(Coin(
                            Enemies[i].get_hitbox()->get_pos(), ddp.normalize()
                        ));
                    }
                } else {
