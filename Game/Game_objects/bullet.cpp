...
Bullet::Bullet(BulletHostType host, Dot from, Dot to, int damage, int speed,
               sprite_t sp)
    : host(host), ddp((to - from).normalize() * speed), damage(damage),
      sprite(sp) {
  pos = from;
}
...
bool Bullet::simulate_attack_on_mob(std::vector<Slime> &Enemies) {
  ...
  for (int i = 0; i < Enemies.size(); i++) {
    if (!Enemies[i].is_invulnerable() &&
        get_collision()->trigger(*Enemies[i].get_hitbox())) {
      // simulate hit
      {
        add_hit_effect(pos);
      ...
      if (Enemies[i].hp <= 0) {
        ...
        if (randomness(20)) {
          game_variables::Loot_hearts.push_back(
              Heart(Enemies[i].get_hitbox()->get_pos(), ddp.normalize()));
...
bool Bullet::simulate_attack_on_player(std::vector<Player> &Players) {
  ...
  for (auto &player : Players) {
    if (!player.is_invulnerable() &&
        get_collision()->trigger(*player.get_hitbox())) {
      // simulate hit
      {
        add_hit_effect(pos);
        player.hp -= damage;
