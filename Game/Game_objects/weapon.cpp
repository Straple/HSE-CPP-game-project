...
Weapon::Weapon(Dot p, weapon_t t, efloat cooldown, int damage)
    : AbstractObject(p), type(t), cooldown(cooldown), cooldown_accum(cooldown),
      damage(damage) {
  switch (type) {
  case GOLDEN_GUN:
    delta = Dot(19, -12);
...
bool Weapon::may_shot() { return cooldown_accum >= cooldown; }
...
void Weapon::GoldenGun_simulate() {
  ...
  if (half == 0) {
    ...
    dulo -= pivot;
    Dot new_dulo{};
    new_dulo.x = dulo.x * cos((15 / 57.2957795) * (-ind)) -
                 dulo.y * sin((15 / 57.2957795) * (-ind));
    new_dulo.y = dulo.x * sin((15 / 57.2957795) * (-ind)) +
                 dulo.y * cos((15 / 57.2957795) * (-ind));
    dulo = new_dulo + pivot;
    if (ind == 6) {
      dulo.y += 1;
    }
    ...
    dulo -= pivot;
    Dot new_dulo{};
    new_dulo.x = dulo.x * cos((15 / 57.2957795) * (-(ind - 7))) -
                 dulo.y * sin((15 / 57.2957795) * (-(ind - 7)));
    new_dulo.y = dulo.x * sin((15 / 57.2957795) * (-(ind - 7))) +
                 dulo.y * cos((15 / 57.2957795) * (-(ind - 7)));
    dulo = new_dulo + pivot;
  }
  if (ind == 26)
    ind = 0;
...
void Weapon::GoldenGun_shot(BulletHostType bullet_host) {
  ...
  dir = dir.normalize();
  game_variables::Bullets.emplace_back(bullet_host, pos + dulo,
                                       pos + dulo + dir, 1, 1000, SP_COIN);
}
...
void Weapon::Rifle_simulate() {
  ...
  if (half == 0) {
    ...
    dulo -= pivot;
    Dot new_dulo{};
    new_dulo.x = dulo.x * cos((15 / 57.2957795) * (-(ind - 5))) -
                 dulo.y * sin((15 / 57.2957795) * (-(ind - 5)));
    new_dulo.y = dulo.x * sin((15 / 57.2957795) * (-(ind - 5))) +
                 dulo.y * cos((15 / 57.2957795) * (-(ind - 5)));
    dulo = new_dulo + pivot;
  }
  if (ind == 22)
    ind = 0;
...
void Weapon::Rifle_shot(BulletHostType bullet_host) {
  ...
  game_variables::Bullets.emplace_back(
      bullet_host, pos + dulo, pos + dulo + dir, 1, 1000, SP_RIFLE_BULLET);
}
...
void Weapon::Sniper_shot(BulletHostType bullet_host) {
  ...
  dir = dir.normalize();
  game_variables::Bullets.emplace_back(
      bullet_host, pos + dulo, pos + dulo + dir, 5, 8000, SP_RIFLE_BULLET);
}
