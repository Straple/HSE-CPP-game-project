...
Barrel::Barrel(const Dot &position) { pos = position; }
...
void Barrel::draw() {
  ...
  // draw_circle(Circle(pos - global_variables::camera.pos - Dot(), 8), Color(0,
  // 128));
...
bool Barrel::trigger_in_draw_sprite(const Dot &check_pos) {
  return collision_in_draw_sprite(pos + delta_draw_pos, size, SP_BARREL,
                                  check_pos);
}
