    ...
    for (auto &object1 : game_variables::Objects) {                                       \
        for (auto &object2 : game_variables::Objects) {                                   \
            if (&object1 != &object2 && (condition)) {                                    \
                object1.push_out_of_collision_soft(*object2.get_collision(), delta_time); \
            }                                                                             \
        }                                                                                 \
    }
...
#define PUSH_OUT_SOFT(Objects, OneWhoPushes, condition)                                        \
    do {                                                                                       \
        for (auto &object : game_variables::Objects) {                                         \
            for (auto &he_pushes : game_variables::OneWhoPushes) {                             \
                if (condition) {                                                               \
                    object.push_out_of_collision_soft(*he_pushes.get_collision(), delta_time); \
                }                                                                              \
            }                                                                                  \
        }                                                                                      \
    } while (0)
    ...
    // player <-> bomber
    PUSH_OUT_SOFT(Players, Bombers, he_pushes.is_booming && !object.is_dead() && !object.is_paralyzed);
