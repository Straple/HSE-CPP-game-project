...
#define PUSH_OUT_HARD_ONE(collision, Objects)         \
    ...
    // сами с собой
    {
        ...
        // возможно object1 нельзя выталкивать: слайм ест игрока, они оба
        // статичны
    ...
    for (auto &object1 : game_variables::Objects) {      \
        for (auto &object2 : game_variables::Objects) {  \
            if (&object1 != &object2 && (condition)) {   \
                object1.push_out_of_collision_soft(      \
                    *object2.get_collision(), delta_time \
                );                                       \
            }                                            \
        }                                                \
    }

        PUSH_OUT_YOURSELF_SOFT(
            Players, !object1.is_paralyzed && !object2.is_dead()
        )
        PUSH_OUT_YOURSELF_SOFT(Slimes, !object1.is_devour)
        PUSH_OUT_YOURSELF_SOFT(
            Bombers, !object1.is_booming && !object2.is_booming
        )
    ...
    do {                                                           \
        for (auto &object : game_variables::Objects) {             \
            for (auto &he_pushes : game_variables::OneWhoPushes) { \
                if (condition) {                                   \
                    object.push_out_of_collision_soft(             \
                        *he_pushes.get_collision(), delta_time     \
                    );                                             \
                }                                                  \
            }                                                      \
        }                                                          \
...
#define PUSH_OUT_HARD(Objects, OneWhoPushes, condition)            \
    do {                                                           \
        for (auto &object : game_variables::Objects) {             \
            for (auto &he_pushes : game_variables::OneWhoPushes) { \
                if (condition) {                                   \
                    object.push_out_of_collision_hard(             \
                        *he_pushes.get_collision()                 \
                    );                                             \
                }                                                  \
            }                                                      \
        }                                                          \
    ...
    PUSH_OUT_SOFT(
        Players, Bombers,
        he_pushes.is_booming && !object.is_dead() && !object.is_paralyzed
    );
