...
void simulate_player(efloat delta_time, Player &player) {
  ...
  player.simulate(delta_time, accum_ddp(BUTTON_A, BUTTON_D, BUTTON_W, BUTTON_S),
                  PRESSED(BUTTON_SPACE));

  // if (PRESSED(BUTTON_MOUSE_L)) {
  //     add_flower_dome_effect(Players[index].cursor_dir + Players[index].pos);
  // }
  if (PRESSED(BUTTON_E) && !player.is_paralyzed && !player.is_jumped &&
      !player.is_dead()) {
    int picked_ind = -1;
    ...
    for (int i = 0; i < game_variables::Weapons.size(); i++) {
      if (game_variables::Weapons[i].is_picked)
        continue;
      auto dist = (player.pos - (game_variables::Weapons[i].pos +
                                 game_variables::Weapons[i].delta))
                      .get_quare_len();
  ...
  if (PRESSED(BUTTON_MOUSE_L) && !player.is_paralyzed && !player.is_jumped &&
      player.coins > 0 && player.weapon_ind != -1 &&
      game_variables::Weapons[player.weapon_ind].may_shot() &&
      !player.is_dead()) {
  ...
  if (IS_DOWN(BUTTON_MOUSE_L) && player.weapon_ind != -1 &&
      game_variables::Weapons[player.weapon_ind].type == RIFLE &&
      !player.is_paralyzed && !player.is_jumped && player.weapon_ind != -1 &&
      game_variables::Weapons[player.weapon_ind].may_shot() &&
      !player.is_dead()) {
    game_variables::Weapons[player.weapon_ind].shot(BulletHostType::PLAYER);
  }
  ...
  // таким образом, мы будем считать, что игрок все еще нажимает на кнопку до
  // тех пор, пока не получим информацию от него об обратном
...
void simulate_game(efloat delta_time) { test_room.simulate(delta_time); }
...
void draw_game(int client_id) {
  ...
  global_variables::arena_half_size.y) * 0.95 + global_variables::camera.pos +
  Dot(i, 0) * 7, 0.5, SP_HEART
          );
      }
      ...
      for (int i = 0; i < player.coins; i++) {
          draw_sprite(
              Dot(-global_variables::arena_half_size.x,
  global_variables::arena_half_size.y) * 0.8 + global_variables::camera.pos +
  Dot(-14 - i * 2 + game_variables::Players[0].coins * 2, 0), 0.5, SP_COIN
          );
      }
  }*/
}
