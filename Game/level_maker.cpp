...
void render_game(const Input &input, const Dot &cursor_pos) {
    ...
    // draw current choice object
    {
        if (current_mode == LM_SPRITE) {
            ...
            if (IS_DOWN(BUTTON_R)) {
                pos.x -=
                    get_sprite(current_type_of_sprite).width() * current_size;
            }

            if (IS_DOWN(BUTTON_T)) {
                pos.y +=
                    get_sprite(current_type_of_sprite).height() * current_size;
            }
    ...
    draw_rect(
        cursor_pos, Dot(1, 1) / global_variables::render_scale / 300, RED
    );
...
void simulate_input(const Input &input, Dot &cursor_pos) {
    ...
    if (PRESSED(BUTTON_MOUSE_R)) {
        if (current_mode == LM_SPRITE) {
            ...
            for (int i = static_cast<int>(current_room.Walls.size()) - 1;
                 i >= 0; i--) {
            ...
            for (int i = static_cast<int>(current_room.ColorBoxes.size()) - 1;
                 i >= 0; i--) {
                if (CollisionBox(
                        std::get<0>(current_room.ColorBoxes[i]),
                        std::get<1>(current_room.ColorBoxes[i])
                    )
                        .trigger(current_pos)) {
                    current_room.ColorBoxes.erase(
                        current_room.ColorBoxes.begin() + i
                    );
                    break;
                }
            }
