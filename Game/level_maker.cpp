...
void render_game(const Input &input, const Dot &cursor_pos) {
  ...
  // draw level
  {
    ...
    for (int i = 0; i < current_room.Draw_objects.size(); i++) {
      ...
      if (need_highlight == i) {
        ...
        if (current_mode == LM_RENDER_LEVEL) {
          draw_object(level, pos - global_variables::camera.pos, 0.5,
                      Color(0xff0000, 128));
        }
      }
    }
    ...
    if (current_mode == LM_INTERESTING_DOT) {
      for (auto [pos, name] : current_room.Interesting_dots) {
        if ((current_pos - pos).get_len() < 1) {
          ...
          draw_text(name.c_str(), pos - global_variables::camera.pos, 0.5,
                    YELLOW);
        } else {
          draw_rect(pos - global_variables::camera.pos, Dot(0.5, 0.5),
                    Color(0xffff00, 128));
          draw_text(name.c_str(), pos - global_variables::camera.pos, 0.5,
                    Color(0xffff00, 128));
        }
    ...
    Bush(Objects[i].first).trigger_in_draw_sprite(current_pos)) ||
                (Objects[i].second == OT_TREE &&
    Tree(Objects[i].first).trigger_in_draw_sprite(current_pos)) ||
                (Objects[i].second == OT_TABLE &&
    Table(Objects[i].first).trigger_in_draw_sprite(current_pos))) {
                need_highlight = i;
                break;
            }
        }
  ...
  // draw current choice object
  {
    if (current_mode == LM_SPRITE) {
      ...
      for (auto [p0, p1, color] : current_room.ColorBoxes) {
        draw_rect2(p0 - global_variables::camera.pos,
                   p1 - global_variables::camera.pos, color);
      }
  ...
  if (current_mode == LM_SPRITE) {
    draw_object(sprite_type_to_string(current_type_of_sprite),
...
void simulate_input(const Input &input, Dot &cursor_pos) {
  ...
  if (PRESSED(BUTTON_MOUSE_R)) {
    if (current_mode == LM_SPRITE) {
      for (int i = static_cast<int>(current_room.Draw_objects.size()) - 1;
        ...
        auto [pos, size, sprite, level] = current_room.Draw_objects[i];
        if (collision_in_draw_sprite(pos, size, sprite, cursor_pos)) {
          current_room.Draw_objects.erase(current_room.Draw_objects.begin() +
                                          i);
          break;
        }
      }
    } else if (current_mode == LM_COLLISION_BOX) {
      for (int i = static_cast<int>(current_room.Walls.size()) - 1; i >= 0;
           i--) {
      ...
      for (int i = 0;
           i < static_cast<int>(current_room.Interesting_dots.size()); i++) {
        if ((current_pos - current_room.Interesting_dots[i].pos).get_len() <
            1) {
      ...
      for (int i = static_cast<int>(Objects.size()) - 1; i >= 0; i--) {
        if ((Objects[i].second == OT_BUSH &&
             Bush(Objects[i].first).trigger_in_draw_sprite(pos)) ||
            (Objects[i].second == OT_TREE &&
             Tree(Objects[i].first).trigger_in_draw_sprite(pos)) ||
            (Objects[i].second == OT_TABLE &&
             Table(Objects[i].first).trigger_in_draw_sprite(pos)) ||
            (Objects[i].second == OT_PILLAR &&
             Pillar(Objects[i].first).trigger_in_draw_sprite(pos)) ||
            (Objects[i].second == OT_NUN_STATUE &&
             NunStatue(Objects[i].first).trigger_in_draw_sprite(pos)) ||
            (Objects[i].second == OT_KNIGHT &&
             Knight(Objects[i].first).trigger_in_draw_sprite(pos)) ||
            (Objects[i].second == OT_BARREL &&
             Barrel(Objects[i].first).trigger_in_draw_sprite(pos))) {
          Objects.erase(Objects.begin() + i);
          break;
        }
      }
    } else if (current_mode == LM_COLOR_BOX) {
      for (int i = static_cast<int>(current_room.ColorBoxes.size()) - 1; i >= 0;
           i--) {
        if (CollisionBox(std::get<0>(current_room.ColorBoxes[i]),
                         std::get<1>(current_room.ColorBoxes[i]))
                .trigger(current_pos)) {
  ...
  // save level
  if (PRESSED(BUTTON_V)) {
    std::sort(current_room.Draw_objects.begin(),
              current_room.Draw_objects.end());
    current_room.write("level.txt", Objects);
  }
}

void simulate_leve_maker(const Input &input, Dot cursor_pos,
                         efloat delta_time) {
  ...
  // simulate camera move
  {
    ...
    auto accum_ddp = [&input](button_t left, button_t right, button_t top,
                              button_t bottom) -> Dot {
      return Dot(IS_DOWN(right) - IS_DOWN(left),
                 IS_DOWN(top) - IS_DOWN(bottom));
    };

    static Dot dp;
    simulate_move2d(global_variables::camera.pos, dp,
                    accum_ddp(BUTTON_A, BUTTON_D, BUTTON_W, BUTTON_S) * 10 /
  ...
  draw_object(current_size, global_variables::arena_half_size + Dot(-20, -20),
              0.5, BLUE);

  {
    ...
    draw_object(mode_str, Dot(0, -global_variables::arena_half_size.y + 5), 0.5,
                RED);
  }
}